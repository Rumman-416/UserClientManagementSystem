...
const AddClient = () => {
  ...
  const handleSubmit = () => {
    if (!data.name) {
      ...
      return toast.warning("Please enter pan number");
    } else if (!data.city) {
      return toast.warning("Please enter city ");
    } else if (!data.industry) {
      return toast.warning("Please enter industry name");
    }
    const formData = {
      name: data.name,
      email: data.email,
      ...
      <div className="rounded bg-white drop-shadow-xl mb-4">
        <div className="lg:p-5 p-4">
          <div className="flex justify-between">
            <h3 className="font-medium mb-5 text-black text-lg">Add Client</h3>
